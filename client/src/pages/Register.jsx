  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (result.type === 'auth/register/fulfilled') {
        toast.success('Welcome to Real Taste Café! 🎉');
      }
    } catch (err) {
      ...
      <div className="max-w-md w-full">
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl card-spacing border-2 border-amber-200">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-serif font-bold text-amber-900">Join Our Community</h2>
            <p className="mt-2 text-amber-700">Create your cafe account ☕</p>
          </div>
          ...
          <form className="space-y-8" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="form-spacing">
              <label htmlFor="name" className="block text-sm font-serif font-semibold text-amber-800 mb-3">
                👤 Full Name
              </label>
              <input
                {...register('name', { 
              ...
              />
              {errors.name && (
                <p className="mt-2 text-sm text-red-600 font-serif">{errors.name.message}</p>
              )}
            </div>
            ...
            <div className="form-spacing">
              <label htmlFor="confirmPassword" className="block text-sm font-serif font-semibold text-amber-800 mb-3">
                🔐 Confirm Password
              </label>
              <input
              ...
              {errors.confirmPassword && (
                <p className="mt-2 text-sm text-red-600 font-serif">{errors.confirmPassword.message}</p>
              )}
            </div>
            ...
            >
              {loading ? '⏳ Creating account...' : '🎉 Join Our Cafe'}
            </button>
            
            <div className="text-center mt-8">
              <p className="text-amber-700 font-serif">
                Already part of our community?{' '}
                <Link to="/login" className="text-amber-800 hover:text-amber-900 font-bold underline">
                  Sign in here ☕
                </Link>
              </p>
            </div>
